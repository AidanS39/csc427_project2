...
with open("test_bnc_corpus.txt", "w") as corpus_file:
    for sentence in new_sents[(num_train+1):(num_test_1)]:
        corpus_file.write(space_delimeter.join(sentence) + "\n")

with open("test_bnc_corpus_2.txt", "w") as corpus_file:
    for sentence in new_sents[(num_test_1+1):(num_sents-1)]:
        corpus_file.write(space_delimeter.join(sentence) + "\n")
